...
const PortfolioContainer = ({ apiData }) => {
    ...
    useEffect(() => {
        getHeldShares()
            .then(shares => setHeldShares(shares))
            .catch(error => console.error('Error fetching held shares:', error));
    }, []);

    useEffect(() => {
        console.log('apiData:', apiData); // Debugging apiData
        const portfolioCurrentPrices =
            heldShares.map((company) => {
                return {
                    ...company,
                    currentPrice: Array.isArray(apiData)
                        ? apiData.filter(stock => stock.symbol === company.symbol)
                                 .map((stock) => stock.price)[0]
                        : null
                };
            });
        setSharesWithPrice(portfolioCurrentPrices);
    ...
    useEffect(() => {
        if (sharesWithPrice.length === 0) {
            setPortfolioTotals({ paid: 0, value: 0 });
            return;
        }
    ...
    const addNewShares = (newShares) => {
        postNewShareAdd(newShares)
            .then(savedNewShares => setHeldShares([...heldShares, savedNewShares]))
            .catch(error => console.error('Error adding new shares:', error));
    };
